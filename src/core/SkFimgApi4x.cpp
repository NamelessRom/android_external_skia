...
int FimgApiStretch(Fimg *fimg, const char *func_name)
{
    ...
    if (FimgApiCheckPossible(fimg) == false) {
    	//SkDebugf("%s FimgApiCheckPossible=false", __func__);
        return false;
    } /* else SkDebugf("%s FimgApiCheckPossible=true", __func__); */

    if (FimgApiIsDstMode(fimg) == true) {
    	// SkDebugf("%s FimgApiIsDstMode=false", __func__);
        return FIMGAPI_FINISHED;
    } /* else SkDebugf("%s FimgApiIsDstMode=true", __func__); */
    ...
    if (FimgApiCompromise(fimg) == false) {
    	//SkDebugf("%s FimgApiCompromise=false", __func__);
        return false;
    } /* else SkDebugf("%s FimgApiCompromise=true", __func__); */
#endif
...
{
	SkDebugf("%s x=%d y=%d w=%d h=%d", __func__, x, y, width, height);
    ...
    fimg.srcColorFormat = 6; //SkBitmap::kARGB_8888_Config;
    ...
    fimg.dstFH          = y + height;
    fimg.dstBPP         = 4; /* 4Byte */
    fimg.dstColorFormat = 6; //SkBitmap::kARGB_8888_Config;
